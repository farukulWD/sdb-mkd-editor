...
const ImageDialog = () => {
  ...
  const [src, setSrc] = useState(state.type === 'editing' ? state.initialValues.src : '')
  const [altText, setAltText] = useState(state.type === 'editing' ? state.initialValues.altText : '')
  const [title, setTitle] = useState(state.type === 'editing' ? state.initialValues.title : '')
  ...
  async function imageUploadHandler(image) {
    ...
    try {
      const response = await axios.post("https://api.bootcampshub.ai/api/settings/watermark-image", formData)
